...
from gui.stats_dialog import StatsDialog, PreviewDialog
...
class ControlPanel(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        self._btn_preview = ctk.CTkButton(
            row2, text="🔬 미리보기", width=100, height=32,
            font=ctk.CTkFont(size=12),
    ...
    def _on_preview(self):
        """미리보기: 현재 설정으로 시뮬레이션 결과를 다이얼로그에 표시."""
        if self._get_settings:
            result = self._get_settings()
            if len(result) == 4:
                timing_cfg, typo_cfg, _, _ = result
            else:
                timing_cfg, typo_cfg, _ = result
        else:
            timing_cfg = TimingConfig()
            typo_cfg = TypoConfig()

        PreviewDialog(self._app, timing_cfg, typo_cfg)
    ...
    def _update_state(self, state: EngineState):
        ...
        self._btn_preview.configure(state="normal" if is_idle else "disabled")
    ...
    def _on_complete(self, stats: dict):
        ...
        # 통계 다이얼로그 표시
        timing_data = self._engine.timing_data if self._engine else []
        try:
            StatsDialog(self._app, stats, timing_data)
        except Exception as e:
            self._append_log(f"[통계 창 오류] {e}")
