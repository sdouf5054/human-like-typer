...
class ControlPanel(ctk.CTkFrame):
    ...
    def __init__(self, master, get_target_text: Callable[[], str], **kwargs):
        super().__init__(master, **kwargs)

        self._get_target_text = get_target_text
        self._root = master  # after() 호출용
    ...
    def _start_hotkey_listener(self):
        ...
        def on_press(key):
            try:
                if key == kb.Key.esc:
                    self._root.after(0, self._on_hard_stop)
                elif key == self._trigger_key:
                    self._root.after(0, self._on_trigger_pressed)
            except Exception:
                pass
    ...
    def _on_start(self, dry_run: bool = False):
        ...
        callbacks = EngineCallbacks(
            on_log=lambda msg: self._root.after(0, self._append_log, msg),
            on_state_change=lambda s: self._root.after(0, self._update_state, s),
            on_progress=lambda c, t: self._root.after(0, self._update_progress, c, t),
            on_countdown=lambda s: self._root.after(0, self._update_countdown, s),
            on_complete=lambda st: self._root.after(0, self._on_complete, st),
        )
