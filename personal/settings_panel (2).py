...
class LabeledSlider(ctk.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._label = ctk.CTkLabel(
            self, text=label,
            font=ctk.CTkFont(size=11), anchor="w", width=140,
        )
        self._label.pack(side="left", padx=(0, 4))

        self._value_label = ctk.CTkLabel(
            self, text="",
            font=ctk.CTkFont(size=11, weight="bold"), anchor="e", width=60,
        )
    ...
    def _update_label(self, value):
        self._value_label.configure(text=f"{value:{self._fmt}}{self._suffix}")
...
class SettingsPanel(ctk.CTkFrame):
    ...
    def _build_timing_tab(self, parent):
        ...
        self._s_base_delay = LabeledSlider(
            scroll, "기본 딜레이", 20, 200, 70, step=5, suffix="ms", on_change=n)
        self._s_base_delay.pack(fill="x", pady=2)

        self._s_variance = LabeledSlider(
            scroll, "딜레이 분산", 0, 80, 30, step=5, suffix="ms", on_change=n)
        self._s_variance.pack(fill="x", pady=2)

        # 단어 경계
        self._sw_word = LabeledSwitch(scroll, "단어 경계 딜레이", True, n)
        self._sw_word.pack(fill="x", pady=2)

        self._s_inter_word = LabeledSlider(
            scroll, "  단어 간 pause", 30, 300, 120, step=10, suffix="ms", on_change=n)
        ...
        # 구두점
        self._sw_punct = LabeledSwitch(scroll, "구두점 pause", True, n)
        self._sw_punct.pack(fill="x", pady=2)

        self._s_punct_pause = LabeledSlider(
            scroll, "  구두점 pause", 50, 500, 200, step=10, suffix="ms", on_change=n)
        self._s_punct_pause.pack(fill="x", pady=2)

        # 개행
        self._sw_newline = LabeledSwitch(scroll, "개행 pause", True, n)
        self._sw_newline.pack(fill="x", pady=2)

        self._s_newline_pause = LabeledSlider(
            scroll, "  개행 pause", 0, 2000, 400, step=50, suffix="ms", on_change=n)
        self._s_newline_pause.pack(fill="x", pady=2)

        # Shift
        self._sw_shift = LabeledSwitch(scroll, "Shift 패널티", True, n)
        self._sw_shift.pack(fill="x", pady=2)

        self._s_shift_penalty = LabeledSlider(
            scroll, "  Shift 추가", 0, 80, 25, step=5, suffix="ms", on_change=n)
        self._s_shift_penalty.pack(fill="x", pady=2)

        # 동일 글자
        self._sw_double = LabeledSwitch(scroll, "동일 글자 가속", True, n)
        ...
        # 버스트
        self._sw_burst = LabeledSwitch(scroll, "버스트 타이핑", False, n)
        self._sw_burst.pack(fill="x", pady=2)

        self._s_burst_pause = LabeledSlider(
            scroll, "  버스트 pause", 10, 100, 40, step=5, suffix="ms", on_change=n)
        self._s_burst_pause.pack(fill="x", pady=2)

        # 피로
        self._sw_fatigue = LabeledSwitch(scroll, "타이핑 피로", True, n)
        self._sw_fatigue.pack(fill="x", pady=2)

        self._s_fatigue_factor = LabeledSlider(
            scroll, "  피로 계수", 0.0, 0.20, 0.05, step=0.01,
            fmt=".2f", on_change=n)
    ...
    def _build_typo_tab(self, parent):
        ...
        self._s_typo_prob = LabeledSlider(
            ...
            fmt=".0f", on_change=self._on_typo_prob_change)
        self._s_typo_prob.pack(fill="x", pady=2)

        self._typo_prob_desc = ctk.CTkLabel(
            scroll, text="  → 0.30% (1000자당 약 3개 오타)",
        ...
        self._s_revision_prob = LabeledSlider(
            scroll, "오타 수정 확률", 0, 100, 85, step=5, suffix="%", on_change=n)
        self._s_revision_prob.pack(fill="x", pady=2)

        ctk.CTkLabel(
            scroll, text="오타 유형:", font=ctk.CTkFont(size=12), anchor="w",
        ).pack(fill="x", padx=4, pady=(8, 2))

        self._sw_adjacent = LabeledSwitch(scroll, "인접 키 오타 (가장 흔함)", True, n)
        self._sw_adjacent.pack(fill="x", pady=2)

        self._sw_transposition = LabeledSwitch(scroll, "글자 전치 오타", False, n)
        self._sw_transposition.pack(fill="x", pady=2)

        self._sw_double_strike = LabeledSwitch(scroll, "이중 입력 오타", False, n)
        self._sw_double_strike.pack(fill="x", pady=2)

    def _on_typo_prob_change(self):
    ...
    def _build_advanced_tab(self, parent):
        ...
        # 입력 모드
        ctk.CTkLabel(
            scroll, text="키 입력 모드:", font=ctk.CTkFont(size=12), anchor="w",
        ...
        ctk.CTkRadioButton(
            mode_frame, text="정교 모드 (Shift press/release)",
        ...
        # 전처리
        ctk.CTkLabel(
            scroll, text="텍스트 전처리:", font=ctk.CTkFont(size=12), anchor="w",
        ).pack(fill="x", padx=4, pady=(12, 2))

        self._sw_normalize_spaces = LabeledSwitch(
            scroll, "연속 공백 정규화 (2개 이상 → 1개)", False, n)
        ...
        ctk.CTkLabel(
            newline_frame, text="개행 처리:", font=ctk.CTkFont(size=11), anchor="w",
        ...
        self._sw_max_length = LabeledSwitch(scroll, "최대 길이 제한", False, n)
        self._sw_max_length.pack(fill="x", pady=2)

        self._s_max_length = LabeledSlider(
            scroll, "  최대 글자 수", 100, 50000, 10000, step=100, suffix="자", on_change=n)
    ...
    # ============================================================
    # Config 로더 (dataclass → 패널) — 프리셋 적용 시 사용
    # ============================================================
